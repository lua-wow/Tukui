...
function UnitFrames:PostUpdateIcon(unit, button, index, position, duration, expirationTime, debuffType, isStealable)
	local data = {
		index = index,
		duration = duration,
	...
	if button then
		if (data.isHarmful) then
			...
			if button.Animation then
				if (isStealable or data.dispelName == "Magic") and UnitIsEnemy("player", unit) then
					local color = isStealable and T.Colors.debuff["Stealable"] or { r = .2, g = .6, b = 1 }
...
function UnitFrames:Enable()
	...
	RegisterStateDriver(Tukui_PetBattleFrameHider or TukuiParent, "visibility", "hide")
end
